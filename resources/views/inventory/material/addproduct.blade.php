                    ...
                    <div class="card-header">
                        <div class="row align-items-center">
                            ...
                            <div class="col-4 text-right">
                                <a href="{{ route('materialused.show', [$materialused->id]) }}" class="btn btn-sm btn-primary">Back to List</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <form method="post" action="{{ route('materialused.material.store', $materialused) }}" autocomplete="off">
                            @csrf

                            <div class="pl-lg-4">
                                <input type="hidden" name="material_u_id" value="{{ $materialused->id }}">
                                <div class="form-group{{ $errors->has('mat_id') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-product">Materials</label>
                                ...
                                <div class="form-group{{ $errors->has('mat_id') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="number" step=0.001 name="qty" id="input-qty" class="form-control form-control-alternative{{ $errors->has('mat_id') ? ' is-invalid' : '' }}" value="0" required>
                                    @include('alerts.feedback', ['field' => 'mat_id'])
                                </div>
